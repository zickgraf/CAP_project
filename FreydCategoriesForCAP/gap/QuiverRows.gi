...
InstallMethod( QuiverRows,
  ...
  function( underlying_quiver_algebra, over_Z )
    ...
    if not over_Z then
        ...
        #return QuiverRowsAsAdditiveClosureOfAlgebroid( underlying_quiver_algebra, over_Z );
...
InstallMethod( QuiverRowsObject,
  ...
  function( list_of_vertices, category )
    local s, L, current, vertex, additive_closure, algebroid, vertices, quiver_rows_object, v;
...
InstallMethod( QuiverRowsMorphism,
  ...
  function( source, matrix, range )
    local category, s, r, objects, quiver_rows_morphism;
...
InstallMethod( \[\],
  ...
  function( object, i )
    ...
    if IsAdditiveClosureCategory( CapCategory( object ) ) then
        
        return ObjectList( object )[i];
...
InstallGlobalFunction( INSTALL_FUNCTIONS_FOR_QUIVER_ROWS,
  ...
  function( category, over_Z )
    ...
    MATRIX_FOR_ALGEBROID_HOMSTRUCTURE := function( v, w, vp, wp, path_1, path_2 )
      local quiver_algebra, basis_elements_source, basis_elements_range, elem_alpha, elem_beta, size_source, size_range, images;
        #local images, hom_v_w, hom_vp_wp, alpha, beta, path;
                                        
        quiver_algebra := algebra;
        
        data := basis_paths_by_vertex_index;
        
        #basis_elements_source := basis_paths_by_vertex_index[ VertexIndex( v ) ][ VertexIndex( w ) ];
        
        #if IsEmpty( basis_elements_source ) then
        #    
        #    return [];
        #    
        #fi;
        
        #basis_elements_range := basis_paths_by_vertex_index[ VertexIndex( vp ) ][ VertexIndex( wp ) ];
        
        #if IsEmpty( basis_elements_range ) then
        #    
        #    return [];
        #    
        #fi;
        
        elem_alpha := PathAsAlgebraElement( algebra, path_1 );
        
        elem_beta := PathAsAlgebraElement( algebra, path_2 );
        ...
        QUIVER_ROWS_COUNTER := QUIVER_ROWS_COUNTER + 1;
        ...
        #        
        #        Add( images,
        #          CoefficientsOfPaths( basis_elements_range, Representative( elem_alpha * PathAsAlgebraElement( algebra, path ) * elem_beta ) )
        #        );
        #        
        #    od;
        #    
        ...
        #        
        #        Add( images,
        #          CoefficientsOfPaths( basis_elements_range, ( elem_alpha * PathAsAlgebraElement( algebra, path ) * elem_beta ) )
        #        );
        #        
        #    od;
        #    
        ...
        images := List( basis_elements_source, path -> CoefficientsOfPaths( basis_elements_range, Representative( elem_alpha * PathAsAlgebraElement( quiver_algebra, path ) * elem_beta ) ) );
    ...
    hom_structure_on_morphisms_for_pure_components := function( alpha, beta, listi, listj, listk, listl )
        ...
        #Display( "######" );
        #Display( VertexIndex(v) );
        #Display( VertexIndex(w) );
        #Display( VertexIndex(vp) );
        #Display( VertexIndex(wp) );
      ...
      function( hom_source, alpha, beta, hom_range )
        ...
        for j in [ 1 .. Size( listj ) ] do
            ...
            for k in [ 1 .. Size( listk ) ] do
                ...
                #if VertexIndex( listj[j][1] ) = 2 and VertexIndex( listk[k][1] ) = 1 then
                ...
                if row_counts[(j-1)*Size( listk ) + k] > 0 then
                    ...
                    for i in [ 1 .. Size( listi ) ] do
                        ...
                        for l in [ 1 .. Size( listl ) ] do
                            ...
                            #if VertexIndex( listi[i][1] ) = 2 and VertexIndex( listj[j][1] ) = 1 and VertexIndex( listk[k][1] ) = 2 and VertexIndex( listl[l][1] ) = 2 then
                            #    
                            #    Error("asd");
                            #    
                            #fi;
