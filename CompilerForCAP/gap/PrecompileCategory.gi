...
InstallGlobalFunction( "CapJitPrecompileCategory", function ( category_constructor, given_arguments, package_name, compiled_category_name )
  local cat1, cat2, cat, obj, mor, operations, diff, output_string, package_info, parameters_string, current_string, object_name, example_input, index, compiled_func, compiled_tree, function_string, filter_list, function_name, current_rec, start_time;
    ...
    for function_name in operations do
        ...
        compiled_tree.nams := Concatenation( current_rec.input_arguments_names, compiled_tree.nams{[ compiled_tree.narg + 1 .. compiled_tree.narg + compiled_tree.nloc ]} );
